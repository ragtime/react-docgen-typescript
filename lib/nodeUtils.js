...
function navigate(node) {
    ...
    var index = 0;
    path.forEach(function (expectedKind) {
        ts.forEachChild(node, function (subNode) {
...
exports.getFlatChildren = getFlatChildren;
function dumpNode(node, prefix) {
    if (prefix === void 0) { prefix = ''; }
    var a = node;
    var info = (a.name ? a.name.text : '')
        || (a.text ? a.text : '');
    console.log(prefix, kindToString(node.kind), '       ' + info);
    ts.forEachChild(node, function (i) { return dumpNode(i, prefix + '    '); });
}
